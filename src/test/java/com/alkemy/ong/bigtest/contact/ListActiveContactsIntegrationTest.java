...
public class ListActiveContactsIntegrationTest extends BigTest {
  ...
  public void shouldReturnListOfContactsWhenUserHasAdminUserRole() throws Exception {
    ...
    mockMvc.perform(get("/contacts")
        ...
        .andExpect(jsonPath("$.contacts[*].id", notNullValue()))
        .andExpect(jsonPath("$.contacts[*].name").value(hasItem("James")))
        .andExpect(jsonPath("$.contacts[*].phone").value(hasItem("159028080")))
        .andExpect(jsonPath("$.contacts[*].message").value(hasItem("my message")))
